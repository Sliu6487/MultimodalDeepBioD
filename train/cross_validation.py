...
class CvTrainTest:
    def __init__(self, multimodal: MultiModels, datasets: dict, folds: int = 5):
        ...
        :param datasets: dataset that is used for hyper-parameter tuning and test
                               tune_datasets = {'X_train_tuple': (X_train_mode1, X_train_mode2),
                                                'X_test_tuple': (X_test_mode1, X_test_mode2),
                                                'y_train': y_train,
                                                'y_test': y_test}
        """
        ...
        # prepare datasets when initialized
    ...
    def cross_validate(self, model_number: int,
        ...
        for fold in range(1, self.folds + 1):
            ...
            if model_number == 3:
                self.multimodal.trained_models['model_trained1'] = self.hist_dict['model1']['trained_models_cv'][fold - 1]
                self.multimodal.trained_models['model_trained2'] = self.hist_dict['model2']['trained_models_cv'][fold - 1]
